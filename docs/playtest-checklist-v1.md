...
- [ ] **weigh_battle against a castle** — ask a party-leading lord "could we take <castle>?".
      *Pass:* a verdict grounded in real strengths (garrison + militia weighed in), composition
      ...
      hedges.
- [ ] **A wife speaking of the children** — ask your spouse about the children / the family.
...
- [ ] **A king receiving a tier-0 stranger** — fresh-ish character (renown < 150, standing < 10
...
- [ ] **"?" info overlay in the chat window (O)** — open it, click "?", read it through.
...
- [ ] **"?" info overlay in the letter window (U)** — same drill.
...
- [ ] **Letter beats in the chat thread** — a correspondence NPC's chat thread shows the
...
- [ ] **Cost notices per exchange** — with `ShowCostNotices` on, each chat exchange ends in one
      soft "✒ Name — message: in → out tokens, calls, ~$" notice; numbers plausible.
- [ ] **No cost notice on sealed flows** — a reach-out desire roll or a letter being composed
...
- [ ] **log.txt filling** — `Configs\ImmersiveAI\log.txt` gathers the session's diagnostics.
- [ ] **First-run popup** — delete `first_run_note_shown.txt`, blank the API key, start the game.
...
- [ ] **gpt-5.6 answering** (only if switching to OpenAI) — luna default; needs the
      max_completion_tokens shape; a plain exchange and a tool-carrying one both answer.

## 4. The map-party farewell fix

- [ ] Click a lord's party on the map → Immersive AI section → Farewell.
      *Pass:* back on the map, NO engage/attack menu.
- [ ] Same talk inside a town.
...
- [ ] Set up the Anthropic key and confirm claude-opus-4-8 answers: a plain exchange, a
      tool-carrying one (recall/heart riding along), and a reflection (memory compression) pass.
