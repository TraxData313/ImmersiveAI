...
|---|---|---|
| `src/LivingCalradia.Core` | netstandard2.0 | Game-independent logic: memory engine, prompt building, LLM abstraction. Fully unit-tested. |
| `src/LivingCalradia.Module` | net472 | The Bannerlord module: campaign behaviors, dialogs, UI. References game DLLs from the local install. |
| `tests/LivingCalradia.Core.Tests` | net8.0 | xUnit tests for Core. |

Key Core concepts:

- `NpcMemory` — three memory layers per NPC: verbatim `RecentTurns`, rolling `Summary`
  (LLM-compressed when turns exceed a threshold), and `KnownFacts` (distilled one-liners).
- `PromptBuilder` — system prompt (persona + scene + memory) plus real user/assistant
  message history.
...
- [ ] M1: Memory & anti-repetition — talk to an NPC in game with layered memory end to end
...
Then enable "Living Calradia" in the Bannerlord launcher.
