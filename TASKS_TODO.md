...
NEXT UPDATE:
- [ ] PLAYTEST: НОЩИТЕ ПО-ДЪЛГИ + РАЖДАНЕТО (built 2026.08.10-11, 450 tests green, unplaytested)
      НОЩИТЕ: дай ѝ подарък от 10, после от 1000, и виж дали разказите растат (3-4 → 7-8 изречения)
      и дали ВТОРАТА половина вече не се затваря на вратата. Ако още завършват еднакво ("той ме
      позна"), кажи ми — значи трябва още натиск в промпта, не повече изречения.
      РАЖДАНЕТО: използвай лоста "[Immersive AI • test — write a child's day anew]" в разговора с
      майка на твое дете (или изчакай истинско раждане). Гледай за, в този ред:
      (1) ❧ известие "X роди син — Име", после ЧАСЪТ, написан от нейно име;
      (2) попъпа "Как ще посрещнеш детето?" — 5 тиера, 100 до 500 000, сивите обяснени;
      (3) ако не си бил при нея, попъпът трябва да дойде чак когато я видиш пак (до 30 дни);
      (4) празникът: гостите, реномето, и ❧ картите в чата — ЧАСЪТ само при нея, ПРАЗНИКЪТ при всички;
      (5) попитай някой ГОСТ за раждането — трябва да ти каже за празника и да откаже часа;
      (6) попитай МАЙКАТА — трябва да ти каже всичко; попитай СЕБЕ СИ през нея за часа като баща —
          трябва да е рамкирано "така ми го разказа тя".
      Файловете: Configs\ImmersiveAI\NPCs\campaign_*\_births\ + births.txt.
- [ ] ПРОВЕРИ СВАТБАТА ЗА 1 000 — вече не пуска непознати. Направи една и виж дали в залата стоят
      само наши хора (и рода ти), без мелничари от града. Ако искаш и нейният род да идва на този
      тиер, кажи — един bool е (`ownKin` е там, родът ѝ нарочно е оставен за 10 000).
