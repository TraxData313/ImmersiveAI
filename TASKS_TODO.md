...
NEXT UPDATE:
- [ ] PLAYTEST: НОЩИТЕ + РАЖДАНЕТО (2026.08.11 — 456 теста зелени, два кръга ревю, промптите ЖИВО
      пробвани с luna върху Сибила; но НИЩО не е пипано в игра)
      НОЩИТЕ — просто ѝ занеси подарък:
      1. За 10 денара → 3-4 изречения. За 1000 → 7-8. Гледай ВТОРАТА половина да остава в стаята,
         а не да се затваря на вратата.
      2. Направи 3-4 платени нощи подред и виж дали ОБРАЗИТЕ са различни (градина, запечатан извор,
         мед под езика, миро по дръжката…). Ако се повтарят — кажи, тестето се удължава за минута.
      3. Ако разказ пак свърши с еднаква формула („той ме позна") — това значи, че трябва още
         натиск в промпта, не повече изречения.
      РАЖДАНЕТО — лост в разговор: „[Immersive AI • test — write a child's day anew]" при майка на
      твое дете (или изчакай истинско раждане):
      4. Известие „X роди син — Име", после ЧАСЪТ, написан от нейно име.
      5. Попъп „Как ще посрещнеш детето?" — 5 тиера, 100 до 500 000. Ако имаш ПОД 100 денара,
         попъпът НЕ трябва да излиза изобщо (това беше бъг, който паузираше света всеки час).
      6. Ако не си бил при нея — попъпът идва чак когато я видиш пак (до 30 дни).
      7. Празник за 1000: в залата САМО наши хора + ДВАТА рода — твоят и нейният. Без мелничари,
         без нобели дошли да се покажат.
      8. H прозорецът: децата стоят НАЙ-ДОЛУ под името на майката — първо часът, после празникът.
         Като отвориш прозореца трябва да ги виждаш веднага, без да скролваш.
      9. Питай ГОСТ за раждането → дава ти празника, отказва часа. Питай МАЙКАТА → дава всичко.
      10. Файлове: Configs\ImmersiveAI\NPCs\campaign_*\_births\ + births.txt.
      ЗНАМ ЗА ЕДНО ДРЕБНО: имена на градове понякога излизат полулатински („Аkkалат"), или „динара"
      вместо „денара" — артефакт на модела при транслитерация, около 1 на 3. Ако те дразни, кажи.
